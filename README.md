...
A function in the JS that waits (‘listens’) for an event to occur and then responds with the predetermined action.

4. Why would we convert a NodeList into an Array?

 When using a Nodelist there is no access to .reduce or .map or any other array method (other than .forEach). So we would use Array.from in order to access those methods.
